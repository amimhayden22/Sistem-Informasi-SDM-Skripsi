<div class="main-sidebar sidebar-style-2">
	<aside id="sidebar-wrapper">
		...
		<ul class="sidebar-menu">
			...
			<li class="{{ (request()->is('dashboard/parts*')) ? 'active' : '' }}">
				<a class="nav-link" href="{{ route('parts.index') }}">
					<i class="fas fa-handshake"></i>
					<span>Bagian</span>
				</a>
			</li>
			<li class="{{ (request()->is('dashboard/positions*')) ? 'active' : '' }}">
				<a class="nav-link" href="{{ route('positions.index') }}">
					<i class="fas fa-map-pin"></i>
					<span>Jabatan</span>
				</a>
			</li>
			<li class="{{ (request()->is('dashboard/employees*')) ? 'active' : '' }}">
			...
			<li class="{{ (request()->is('dashboard/work-permit*')) ? 'active' : '' }}">
				<a class="nav-link" href="{{ route('work-permit.index') }}">
					<i class="fas fa-calendar-alt"></i>
					<span>Pengajuan Izin Kerja</span>
				</a>
			</li>
		</ul>
		<div class=" mb-4 p-3 hide-sidebar-mini">
          ...
          document.getElementById('logout-form').submit();">
				<i class="fas fa-sign-out-alt"></i> Logout <form id="logout-form" action="{{ route('logout') }}" method="POST" class="d-none"> @csrf </form>
			</a>
		</div>
	</aside>
</div>
